...
export default function BudgetVisualization({ budget, currency = '€' }) {
  ...
  if (!budget || !budget.scenarios) {
    return (
      <div className={styles.empty}>
        <div className={styles.emptyIcon}>BD</div>
  ...
  const formatMoney = (value) => `${currency}${Number(value || 0).toLocaleString(undefined, { maximumFractionDigits: 0 })}`;
          ...
          {scenarios.map((scenario, idx) => (
            ...
            >
              <span className={styles.scenarioLabel}>
                {scenario.tier === 'economical' && 'Eco'}
                {scenario.tier === 'balanced' && 'Bal'}
                {scenario.tier === 'premium' && 'Pro'}
              </span>
              ...
              <span className={styles.scenarioPrice}>{formatMoney(scenario.total)}</span>
            </button>
          ))}
        </div>
      )}
      ...
      <div className={styles.totalOverview}>
        ...
        {active.perDay && (
          <div className={styles.perDay}>
            ~{formatMoney(Math.round(active.perDay))}/dia
          </div>
        )}
      </div>
          ...
          return (
            <div key={key} className={styles.breakdownItem}>
              <div className={styles.breakdownHeader}>
                ...
                <span className={styles.breakdownValue}>{formatMoney(value)} ({percentage}%)</span>
              </div>
        ...
        <div className={styles.comparisonSection}>
          ...
          <div className={styles.comparisonTable}>
            ...
            <div className={styles.comparisonRow}>
              ...
              {scenarios.map((s, idx) => (
                <div key={idx} className={styles.comparisonCell}>
                  {formatMoney(s.perDay || Math.round(s.total / 3))}
                </div>
              ))}
            </div>
          </div>
